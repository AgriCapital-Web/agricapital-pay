...
const ClientPayment = ({ souscripteur, plantations, paiements, onBack, prefillAmount, prefillType }: ClientPaymentProps) => {
  ...
  const TARIFS = useMemo(() => {
    ...
    if (offre) {
      ...
      return {
        jour: Math.round(cm / 30), semaine: Math.round(cm / 4), mois: cm,
        trimestre: cm * 3, semestre: cm * 6, annee: cm * 12,
    ...
    onSuccess(async (response) => {
      if (currentPaiementRef) {
        const { data: paiementData } = await supabase
          .from('paiements').select('*, plantations(*)').eq('reference', currentPaiementRef).maybeSingle();
  ...
  const calculerMontantAvance = () => {
    ...
    return TARIFS[avancePeriodType] * avancePeriodCount * superficie;
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast({ variant: "destructive", title: "Erreur", description: error.message }); setLoading(false);
    }
  };
          ...
          <Card className="border-0 shadow-md">
            <CardContent className="p-5 space-y-4">
              <div className="flex items-center gap-2 mb-1">
                ...
                <h3 className="text-base font-bold">Type de paiement</h3>
              </div>
              <RadioGroup value={typePaiement} onValueChange={(v) => setTypePaiement(v as any)} className="space-y-3">
                {[
                  ...
                  <div key={opt.val} onClick={() => setTypePaiement(opt.val as any)}
                    className={`flex items-center gap-3 p-4 rounded-2xl border-2 cursor-pointer transition-all ${typePaiement === opt.val ? 'border-primary bg-primary/5 shadow-sm' : 'border-border hover:border-primary/30'}`}>
                    ...
                    <Badge variant="secondary" className="text-[10px] shrink-0">{opt.badge}</Badge>
                  </div>
                ))}
              </RadioGroup>
              <Button onClick={() => setStep('plantation')} className="w-full h-12 rounded-xl font-bold">
          ...
          <Card className="border-0 shadow-md">
            <CardContent className="p-5 space-y-4">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-base font-bold">Choisir la plantation</h3>
              </div>
              {plantations.length === 0 ? (
                <div className="text-center py-10 text-muted-foreground">
                  <AlertTriangle className="h-12 w-12 mx-auto mb-3 text-accent" />
                  <p className="font-semibold">Aucune plantation</p>
                  <p className="text-xs mt-1">Contactez votre technico-commercial</p>
                </div>
              ) : (
                    ...
                    return (
                      <div key={plant.id} onClick={() => isSelectable && setSelectedPlantation(plant.id)}
                        className={`p-4 rounded-2xl border-2 transition-all ${
                          selectedPlantation === plant.id ? 'border-primary bg-primary/5 shadow-sm' :
                          isSelectable ? 'border-border hover:border-primary/30 cursor-pointer' : 'border-border opacity-40'
                        }`}>
                        <div className="flex justify-between items-start mb-2">
                          ...
                          {selectedPlantation === plant.id && <div className="h-6 w-6 rounded-full bg-primary flex items-center justify-center"><Check className="h-3.5 w-3.5 text-white" /></div>}
                        </div>
                        ...
                        {typePaiement === 'redevance' && plant.date_activation && (
                          <div className={`flex items-center gap-1.5 text-[11px] mt-2 p-2 rounded-xl ${etat.enAvance ? 'bg-green-50 text-green-700' : etat.jours > 0 ? 'bg-red-50 text-red-700' : 'bg-muted/30 text-muted-foreground'}`}>
              ...
              {selectedPlantation && (
                <Button onClick={() => setStep('details')} className="w-full h-12 rounded-xl font-bold">Continuer</Button>
              )}
            </CardContent>
          </Card>
        )}
          ...
          <Card className="border-0 shadow-md">
            <CardContent className="p-5 space-y-4">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-base font-bold">{typePaiement === 'da' ? "Droit d'Accès" : 'Détails du paiement'}</h3>
              </div>

              <div className="bg-muted/30 rounded-2xl p-3 flex items-center gap-3">
              ...
              {typePaiement === 'da' ? (
                ...
                <div className="space-y-4">
                  {(() => { const etat = calculerArrieres(plantation); if (!etat.jours && !etat.enAvance) return null; return (
                    <div className={`p-3 rounded-2xl border ${etat.enAvance ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'}`}>
                      <div className={`flex items-center gap-2 font-bold text-sm ${etat.enAvance ? 'text-green-700' : 'text-red-700'}`}>
                        {etat.enAvance ? <Trophy className="h-4 w-4" /> : <AlertTriangle className="h-4 w-4" />}
                        {etat.enAvance ? 'En avance !' : 'Arriéré détecté'}
                      </div>
                      <p className={`text-xs mt-0.5 ${etat.enAvance ? 'text-green-600' : 'text-red-600'}`}>
                  ...
                  {(modeArriere || montantArriere > 0) && (
                    <div className="space-y-2">
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <div onClick={() => setModeArriere('only')} className={`p-3 rounded-2xl border-2 cursor-pointer text-center transition-all ${modeArriere === 'only' ? 'border-primary bg-primary/5' : 'border-border hover:border-primary/30'}`}>
                  ...
                  {modeArriere === 'avance' && (
                    <div className="space-y-3 p-3 bg-primary/5 rounded-2xl border border-primary/15">
                      ...
                      <div className="flex justify-between text-sm text-primary font-bold">
                        <span>Avance :</span><span>{fmt(montantAvance)}</span>
                      </div>
                    </div>
                  )}
                      ...
                      <div className="grid grid-cols-3 gap-1.5">
                        {PERIODE_OPTIONS.map(opt => (
                          <div key={opt.key} onClick={() => { setPeriodType(opt.key); setPeriodCount(1); }}
                            className={`p-2 rounded-xl border-2 cursor-pointer text-center transition-all ${periodType === opt.key ? 'border-primary bg-primary/5' : 'border-border hover:border-primary/30'}`}>
                            <p className="text-[11px] font-bold">{opt.label}</p>
                            <p className="text-[9px] text-muted-foreground">{fmt(opt.tarif)}</p>
                          </div>
                        ))}
                      </div>
                      <div onClick={() => setPeriodType('custom')} className={`p-2.5 rounded-xl border-2 cursor-pointer text-center ${periodType === 'custom' ? 'border-primary bg-primary/5' : 'border-border'}`}>
              ...
              {/* Total */}
              <div className="rounded-2xl p-4" style={{ background: 'linear-gradient(135deg, #00643C10, #E89C3115)' }}>
                {modeArriere === 'avance' && (
              ...
              <Button onClick={() => setStep('confirm')} disabled={montantTotal <= 0} className="w-full h-12 rounded-xl font-bold" style={{ background: 'linear-gradient(135deg, #E89C31, #d4872a)', color: '#fff' }}>
        ...
        {step === 'confirm' && plantation && (
          <Card className="border-0 shadow-md">
            <CardContent className="p-5 space-y-4">
              ...
              <div className="bg-primary/5 rounded-2xl p-4 space-y-2.5 text-sm">
              ...
              <Button onClick={handleSubmit} disabled={loading} className="w-full h-14 text-base rounded-xl font-bold" style={{ background: 'linear-gradient(135deg, #E89C31, #d4872a)', color: '#fff' }}>
